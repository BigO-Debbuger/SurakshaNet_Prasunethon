...
function BlockchainDemo() {
  ...
  const simulateAttack = async () => {
    ...
    setProcessSteps(prev => [...prev, `Threat Severity Determined: ${severity} (Confidence: ${(Math.random() * 0.3 + 0.7).toFixed(2)})`])
    setCurrentStep(3)
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 1000))
      setProcessSteps(prev => [...prev, 'Mining Block with Proof of Work...'])
      setCurrentStep(6)
            ...
            {processSteps.map((step, index) => (
              ...
              >
                <div className="step-icon">
                  {index < currentStep - 1 ? '[DONE]' : index === currentStep - 1 ? '' : '[ ]'}
                </div>
          ...
          {blocks.map((block, index) => (
            ...
            >
              ...
              <div className="block-chain-link">
                {index < blocks.length - 1 && <div className="chain-arrow">|</div>}
              </div>
            </div>
          ))}
        </div>
