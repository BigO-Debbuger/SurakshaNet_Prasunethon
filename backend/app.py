...
try:
    ...
    try:
        ...
        print(f"  Could not initialize SHAP explainer: {shap_error}")
except Exception as e:
...
try:
    ...
    print(f"  Could not load Autoencoder model: {e}")
...
if not scaler or not isolation_forest_model or not autoencoder_model or not xgboost_classifier:
    print("  Some models are missing. Running in partial/demo mode.")
...
Immediate action required!"""
        ...
        print(f"💬 Slack alert sent response: {response}")
        return True
...
SurakshaNet Security Operations"""
        ...
        print(f"📧 Gmail alert response: {response}")
        return True
    ...
    try:
        ...
        try:
            ...
            if severity in ["MEDIUM", "HIGH", "CRITICAL"]:
                print(f"  High severity detected: {severity}")
...
def mitigate_threat_route():
    ...
    try:
        ...
        try:
            ...
            print(f"  TAKING ACTION: Blocking IP {source_ip}")
...
# === HEARTBEAT SENDER ===
def start_heartbeat():
    """Sends a UDP pulse to the Raspberry Pi to prove system health"""
    # REPLACE WITH YOUR RASPBERRY PI'S IP ADDRESS
    PI_IP = "192.168.137.99" 
    PI_PORT = 9999

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    print(f" Heartbeat Service Started -> Targeting {PI_IP}:{PI_PORT}")

    while True:
        try:
            sock.sendto(b"ALIVE", (PI_IP, PI_PORT))
...
if __name__ == '__main__':
    ...
    print("="*70)
    print("\n  Make sure blockchain server is running on port 3001!\n")
